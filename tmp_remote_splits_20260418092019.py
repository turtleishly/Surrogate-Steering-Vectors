...
class SplitConfig:
    ...
    # New API: reserve one or more collection names for holdout.
    forced_holdout_collections: Optional[Sequence[str]] = ("execute_malware",)
    # Backward compatibility for older call sites using a single collection.
    forced_holdout_collection: Optional[str] = None


def _resolve_forced_holdout_names(config: SplitConfig) -> set[str]:
    names: set[str] = set()

    if config.forced_holdout_collections is not None:
        for value in config.forced_holdout_collections:
            text = str(value).strip()
            if text:
                names.add(text)

    if config.forced_holdout_collection is not None:
        text = str(config.forced_holdout_collection).strip()
        if text:
            names.add(text)
...
def assign_grouped_splits(index_df: pd.DataFrame, config: SplitConfig) -> pd.DataFrame:
    ...
    forced_holdout_names = _resolve_forced_holdout_names(config)
    if forced_holdout_names:
        forced_mask = work[config.collection_col].astype(str).isin(forced_holdout_names)
